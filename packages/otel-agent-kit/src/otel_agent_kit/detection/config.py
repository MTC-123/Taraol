...
class WatcherConfig:
    ...
    def from_env(cls) -> "WatcherConfig":
        ...
        return cls(
            ...
            loop_window_sec=_positive_int("OAK_LOOP_WINDOW_SEC", 30),
            loop_max_repeats=_positive_int("OAK_LOOP_MAX_REPEATS", 3),
            budget_usd=_positive_float("OAK_BUDGET_USD", 0.01),
            poll_interval_sec=_positive_int("OAK_POLL_INTERVAL_SEC", 5),
            budget_lookback_sec=_positive_int("OAK_BUDGET_LOOKBACK_SEC", 3600),
            signal_cooldown_sec=_positive_int("OAK_SIGNAL_COOLDOWN_SEC", 60),
            otlp_endpoint=os.environ.get("OTEL_EXPORTER_OTLP_ENDPOINT", "http://localhost:4317"),
            signoz_clickhouse_url=clickhouse_url,
            breaker_edge_max=_positive_int("OAK_BREAKER_EDGE_MAX", 10),
            xconv_min_repeats=_positive_int("OAK_XCONV_MIN_REPEATS", 1),
            loop_iteration_hard_cap=_positive_int("OAK_LOOP_ITERATION_HARD_CAP", 12),
        )
