"""Cycle detection over span trees and over aggregated edge graphs.

``find_cycles`` walks a single trace's parent/child span tree (deliberately ignoring
sibling fan-out).  ``find_directed_cycles`` enumerates elementary cycles in an
aggregated directed edge multiset — used for cross-conversation loops that no single
trace tree can see.
"""
...
def find_cycles(spans: Iterable[dict[str, Any]], max_repeats: int = 3) -> list[Cycle]:
    """Return unique ancestry cycles within one trace; never merge sibling paths."""
...
def find_directed_cycles(
    ...
    def visit(start: str, node: str, path: list[str], on_path: set[str]) -> None:
        for nxt in sorted(adjacency[node]):
            if nxt == start:
                record(path)
            elif nxt > start and nxt not in on_path:
                visit(start, nxt, [*path, nxt], on_path | {nxt})
